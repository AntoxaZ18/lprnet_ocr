...
def create_dataloader(image_dir:str=None, batch_size: int=8, transforms=None, shuffle: bool = True) -> DataLoader:
    """
    create dataloader
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument("--proceed", type=bool, help="True for continue from saved checkpoint")
    ...
    train_transform = transforms.Compose([
        transforms.Resize(image_size, antialias=True, interpolation=transforms.InterpolationMode.BICUBIC),
    ...
    val_transform = transforms.Compose([
        transforms.Resize(image_size, interpolation=transforms.InterpolationMode.BICUBIC),     # сохранить высоту ширину — пропорционально
        transforms.ToTensor(),             # преобразование в тензор
        transforms.Normalize(mean=mean, std=std),
    ])

    train_dataloader = create_dataloader(image_dir="train/img", trainsforms=train_transform, batch_size=batch_size)
    val_dataloader = create_dataloader(image_dir="val/img", trainsforms=val_transform, batch_size=batch_size, shuffle=False)
    ...
    train_loop(model, STN, train_dataloader, val_dataloader, criterion, optimizer, scheduler, start_epoch, epochs, CHECKPOINT_PATH)
